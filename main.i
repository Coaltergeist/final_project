...
void drawSprites() {
 ...
 shadowOAM[0].attr1 = (0x1FF & hero.screenCol) | (3<<14);
    ...
    for (int i = 0; i < 1; i++) {
        shadowOAM[i + 1].attr0 = (0xFF & obstacles[i].screenRow) | (2<<14);
        shadowOAM[i + 1].attr1 = (0x1FF & obstacles[i].screenCol) | (2<<14);
        shadowOAM[i + 1].attr2 = ((24)*32+(4));
    }
...
void initialize() {
 ...
 (*(unsigned short *)0x4000000) = 0 | (1<<10) | (1<<9) | (1<<12) | (1<<8);
 ...
 DMANow(3, bgTiles, &((charblock *)0x6000000)[0], 14400 / 2);
    ...
    (*(volatile unsigned short *)0x04000012) = -9;
